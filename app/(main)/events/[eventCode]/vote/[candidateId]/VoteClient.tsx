...
export default function VoteClient({ event, candidate }: VoteClientProps) {
  ...
  const handlePayment = () => {
    // Navigate to payment confirmation with reference
    const ref = `REF-${Math.floor(Math.random() * 1000000)}`;
    const query = new URLSearchParams({
      amount: totalAmount,
      votes: voteCount.toString(),
      candidate: candidate.name,
      // Pass these new fields too if needed for backend/confirmation
      voterName: fullName,
      voterPhone: phoneNumber,
    });

    router.push(`/vote/confirm/${ref}?${query.toString()}`);
  };
        ...
        <div className="md:w-1/2 p-8 md:p-12 flex flex-col justify-center relative">
          ...
          <div className="space-y-6">
            ...
            <div className="text-sm text-gray-500 pt-2">
              Total Price:{" "}
              <span className="font-bold text-gray-900">GHS {totalAmount}</span>
            </div>

            {/* Submit Button */}
            <button
              onClick={handlePayment}
              className="w-full bg-primary-900 text-white font-bold py-4 rounded-full shadow-lg hover:bg-primary-800 transition-transform active:scale-[0.99] mt-4"
            >
              Submit
            </button>
          </div>
